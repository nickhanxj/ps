...
public class CostRecordDao extends BaseDao<CostRecord> {
	...
	public Map<String, Object> monthTotal(String year, String month, String groupId){
		...
		//月消费总额
		String totalCostPerMonthSql = "select round(sum(cost),2) from t_costrecord tc where 1 = 1 "+byYearAndMonth;
		...
		//月消费总额扣除已结算
		String totalCostPerMonthExceptSettledSql = "select round(sum(cost),2) from t_costrecord tc where tc.status = 0 "+byYearAndMonth;
	...
	public Map<String, Object> statisticPerson(String year, String month, String user){
		...
		String costTotalSql = "select round(sum(cost),2) csum,round(avg(cost),2) cavg from t_costrecord tc where tc.user = '"+user+"'"+byYearAndMonth;
		Query costTotal = getSession().createSQLQuery(costTotalSql).setResultTransformer(CriteriaSpecification.ALIAS_TO_ENTITY_MAP);
		rMap.put("costTotal", costTotal.uniqueResult());//消费总金额 
		...
		String settledCostSql = "select round(sum(cost),2) from t_costrecord tc where tc.status = 1 and tc.user = '"+user+"'"+byYearAndMonth;
		...
		String unsettledCostSql = "select round(sum(cost),2) from t_costrecord tc where tc.status = 0 and tc.user = '"+user+"'"+byYearAndMonth;
